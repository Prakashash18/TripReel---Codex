...
This directory is a dependency-light TypeScript Cloudflare Worker for Memories' explicit AI Director. It accepts selected reduced JPEG previews and asks a configurable OpenAI image-capable model for a bounded editorial plan. It does not contain or expose the provider key to the app.
...
### Retired AI video routes

The former `/v1/video/generate`, `/v1/video/status`, and `/v1/video/content` routes return `410 feature_retired` without calling an upstream provider. They remain recognizable temporarily so older TestFlight builds fail clearly and safely.
...
- `test/worker.test.mjs` and `test/video-retirement.test.mjs` — dependency-free contract tests with mocked upstreams.
