...
6. Added the authenticated `delete-account` function. It removes every owned video from private Storage, then deletes the Auth user so profile, link, and allowance rows cascade away.
...
5. Deploy `delete-account` with JWT verification enabled. It validates the caller's bearer token again before using the service role for irreversible cleanup.
