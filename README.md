...
The app points at `https://tripreel-visual-analysis.tripreel-prakashash18.workers.dev/v1/analyze`. This source revision validates every image and model response, disables request logging/storage, caps batches at 24 thumbnails and 128 KiB per thumbnail, and requests strict structured output with low-detail images, no reasoning, and `store: false`. The model defaults to `gpt-5.6-luna` but can be changed only through the Worker's validated `OPENAI_MODEL` setting. This repository task does not deploy the revised Worker; the live endpoint must be updated separately before end-to-end testing the new contract.
