...
Before building a film, Smart Selection runs on the iPhone with Apple Vision. It combines Apple's screenshot subtype, image classification, OCR coverage (recognized text itself is not retained), face and person rectangles, attention saliency, document segmentation, feature prints, and iOS 18+ aesthetics. Human bounds take priority over generic saliency when choosing a starting frame, and photos containing people default to a fit-based treatment so faces and bodies near an edge remain visible. Screenshots do not make a photo collection qualify as a trip. High-confidence utility and very low-quality images are moved to a recoverable **More Photos** list and are never deleted. Perceptually similar photos captured within the same short moment are clustered before highlight selection; TripReel keeps the strongest expression/composition and leaves the alternatives recoverable in **More Photos**. Large trips are then shaped into a concise first cut with day coverage, strong-memory protection, near-duplicate suppression, and visual/category variety; 431 eligible photos produce roughly 64 highlights rather than a nearly nine-minute camera-roll replay. Analysis automatically retries an iCloud thumbnail and then falls back to source-data delivery while decoding only a temporary 512 px copy. A Photos, Vision, or cloud miss that remains stays recoverable in **More Photos** but remains outside the active cut until a later retry can read it, so the first watch contains only working frames. The preview appears first with an optional positive summary and a user-initiated **Check again now** action.

The analysis screen is an immersive live contact-sheet experience: the current photo, recent processed frames, changing Vision phase, and exact progress remain visible while the first cut is assembled. The same local analysis drives an editorial sequence rather than a plain timestamp slideshow: days stay in story order, while strong establishing shots open each day and the cut alternates scenery, people, food, orientation, and near-duplicates. The montage automatically chooses full-bleed, cinematic, portrait-matte, and postcard treatments, and alternates zoom-in, zoom-out, directional pan, rise, and settle motion. Portrait frames preserve the source aspect ratio, use substantially more of the screen, and begin with a conservative face- or subject-aware crop that is identical in preview and export. After First Watch, **Edit your film** opens one Film Studio instead of forcing a refine-then-edit sequence. The vertical film stays completely unobstructed; its preview and sound controls sit beneath the frame. The screen presents only two primary decisions: a native **Edit film** menu for Photos, Framing, Style, Titles, Music, and Pace, and **Export film**. Every editor returns to the same studio. Titles now open as one Reels-style visual workspace: Opening, Place, and End appear together on a horizontal timeline, and each has independently editable title text, supporting text, visibility, type treatment, and time on screen with a live export-matched canvas. A persistent Back control, left-edge swipe, and VoiceOver escape action provide predictable reverse navigation without competing with the screen's primary actions; Export remembers whether it was opened from First Watch or Film Studio. Style choices use a live vertical preview that updates on every tap, while a focused framing editor adds pinch-to-zoom, drag-to-reframe, frame treatment, motion direction, and individual time-on-screen controls. **Reset to Auto** restores the local Vision frame. iOS Reduce Motion always wins.
...
Cloud visual analysis is explicit opt-in and fail-closed. Before the first possible upload, the app presents the choice as a four-step flow: Apple Vision checks locally first, only uncertain reduced 512px JPEG thumbnails can go to OpenAI's `gpt-5.6-luna`, TripReel uses the answer and discards its temporary copy, and on-device Smart Selection remains available if the user declines. The copies are re-encoded without EXIF, GPS, filenames, or stable PhotoKit identifiers. Known screenshots and sensitive/document-like images are blocked from cloud review. Closing or declining the consent screen means no upload.
