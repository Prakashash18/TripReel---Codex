...
The analysis screen is an immersive live contact-sheet experience: the current photo, recent processed frames, changing Vision phase, and exact progress remain visible while First Cut is assembled. When First Cut finishes playing, the last frame is held and the film itself carries the offer: the title, what it is made of, and one action, **Keep this one**, with the Story Pass price from StoreKit. Beneath it sit three quiet alternatives — **Watch again**, **Another take**, and **Change it** — and **Change it** is the only way to the routing screen, where **Improve with AI**, **Edit Myself**, and **Export This** still live. Manual editing opens a vertical creator workspace with a live 9:16 canvas, draggable photo timeline, visible title and music tracks, and one-tap crop, text, music, style, and pace tools. Improve with AI first shows a short, explicit OpenAI sharing decision. Only after permission does the user choose the exact photos, an optional short story hint, and an on-device recommended reel recipe; no preview is prepared until they tap **Create AI cut**. A successful request creates a separate **AI Cut** while preserving First Cut. The director applies a story arc, editable opening, mid-film chapter and optional ending, one of Memories' bundled soundtracks, a visual treatment, pacing, order, and varied motion choreography. Its hook, chapter, and payoff can also become short text beats placed directly over the relevant photos; every line, placement, style, and entrance remains editable, and the same overlays appear in preview and export. The comparison plays the selected soundtrack and explains the director's concrete recommendations; the user can preview either version, adopt it, edit every applied choice, or try another direction. Failure or cancellation always returns safely to First Cut. Export remains entirely local.
...
Previewing and editing the complete reel are free. The free export is a separate 720p watermarked **Memory Preview**: an on-device planner selects three to five high-quality, relevant moments across the story and gives them brisk trailer pacing. A consumable Story Pass unlocks the full-length 1080p export without a watermark for that memory. It is offered at the end of First Cut, on a sheet raised over the still-playing film, and buying it renders the full story immediately; declining sends the free trailer instead. The export screen's paywall remains for exports started later, and a pending export there automatically resumes after a successful purchase.

Create a RevenueCat project for the iOS app, add the Story Pass consumable in App Store Connect, and put it in the current offering. The consumable must not grant a RevenueCat entitlement. Add the **public iOS SDK key** as the `REVENUECAT_PUBLIC_SDK_KEY` build setting for the TripReel app target. RevenueCat's public SDK key is designed to ship in the app; never use a RevenueCat secret API key there. The displayed price comes from StoreKit through RevenueCat, so it stays localized and is not hard-coded in Swift.

Required dashboard setup before a TestFlight purchase test:

1. Create the consumable Story Pass product in App Store Connect; finish its price, localization, tax, and review metadata.
2. Connect the App Store app to RevenueCat and import Story Pass. Do not attach the consumable to an entitlement.
3. Put Story Pass in RevenueCat's **current offering** using the custom package identifier `story_pass`. Remove subscription and lifetime packages from this offering. If you use another identifier, set `REVENUECAT_STORY_PASS_PACKAGE_ID` in the app target's Build Settings.
...
5. Upload a new TestFlight build. Purchases there use Apple's sandbox accounts. Story Pass is a consumable and its per-memory unlock is retained locally on the purchasing device; it is not presented as a restorable subscription.
